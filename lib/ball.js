...
import { findMostActiveSession, isDesktopSessionPath, lastUserMessageText, listRecentSessions } from './ball-session.js';
...
async function publicSessionPaths(ctx) {
  ...
  try {
    const result = await ctx.bus.request('session:list', {}, { timeoutMs: 5000 });
    ...
    for (const item of sessions) {
      ...
      if (typeof sessionPath === 'string' && sessionPath) paths.add(path.normalize(sessionPath));
    }
    return paths;
  } catch {
    return null;
...
async function currentTarget(ctx) {
  ...
  if (!(allowedPaths instanceof Set)) return null;
  const target = findMostActiveSession({ hanaHome: HANA_HOME, allowedPaths });
...
async function sessionTitleMaps(ctx) {
  const now = Date.now();
  if (sessionTitlesCache && now - sessionTitlesCacheAt < SESSION_TITLES_CACHE_MS) return sessionTitlesCache;
...
async function listSessions(ctx, limit = 5) {
  const allowedPaths = await publicSessionPaths(ctx);
  if (!(allowedPaths instanceof Set)) return [];
  const scanned = listRecentSessions({ hanaHome: HANA_HOME, allowedPaths, limit: Math.max(limit * 3, 24) });
  const byPath = new Map(scanned.map((item) => [path.normalize(item.sessionPath), item]));
  try {
    if (!ctx?.bus || typeof ctx.bus.request !== 'function') return scanned.slice(0, limit);
    const result = await ctx.bus.request('session:list', {}, { timeoutMs: 5000 });
    const sessions = Array.isArray(result) ? result : result?.sessions;
    if (!Array.isArray(sessions)) return scanned.slice(0, limit);
    const named = sessions
      ...
      .map((item) => {
        ...
        if (visibility !== 'public' || item.ownerPluginId) return null;
        ...
        return {
          ...
          lastUserTime: matched?.lastUserAt || 0,
        };
      })
      .filter(Boolean)
...
async function recentSessionPath(ctx, requestedPath = '') {
  ...
  if (candidate) {
    ...
    const current = await resolveTarget(ctx);
    if (!current || path.normalize(current.sessionPath) !== path.normalize(candidate)) return null;
    try {
...
async function submitBallFeedbackInternal(ctx, body = {}) {
  ...
  const feedback = rawFeedback === null || rawFeedback === 'clear' ? 'clear' : String(rawFeedback || '').trim();
  ...
  if (typeof body.sessionId === 'string' && body.sessionId.trim()) {
    ...
    const titles = await sessionTitleMaps(ctx);
    if (!titles.publicIds.has(sessionId)) {
  ...
  if (!record) {
    return { ok: false, status: 404, error: '没有找到对应的配图记录' };
  }
  ...
  const result = await applyPreferenceFeedback({
    ...
    agentId: record.agentId || body.agentId || 'default',
    contextEmotion: record.emotion || '',
    contextKeywords: '',
    restoreSnapshot: record.feedback ? record.feedbackBase : null,
  });
  if (!result.ok) return result;

  const nextFeedback = feedback === 'clear' ? null : feedback;
  const nextBase = nextFeedback ? (record.feedbackBase || result.snapshot) : null;
  ...
  if (!updated.ok) {
    // 偏好文件已经写入时，最近记录更新失败要尽量补偿，避免用户看到“未反馈”却无法撤销。
    try {
      if (record.feedback) {
        await applyPreferenceFeedback({
          dataDir,
          stickerId,
          feedbackType: record.feedback,
          agentId: record.agentId || body.agentId || 'default',
          contextEmotion: record.emotion || '',
          contextKeywords: '',
          restoreSnapshot: record.feedbackBase || null,
        });
      } else {
        await applyPreferenceFeedback({
          dataDir,
          ...
          contextEmotion: record.emotion || '',
          contextKeywords: '',
          restoreSnapshot: result.snapshot || null,
        });
      }
    } catch (error) {
  ...
  return {
    ...
    match: updated.match,
    message: feedback === 'clear' ? '已撤销这次反馈' : (feedback === 'positive' ? '已记下：喜欢' : '已记下：不喜欢'),
  };
}
...
async function writeBallStickerToLibrary(ctx, { imageBase64, fileName, tags = {} }) {
  ...
  return await enqueueToolWrite(async () => {
    ...
    const id = genId();
    const file = `${id}.${ext}`;
    const dest = path.join(STICKERS_DIR, file);
    try {
...
async function confirmBallRecognition(ctx, body = {}) {
  ...
  const written = await writeBallStickerToLibrary(ctx, { imageBase64, fileName, tags });
