...
// v0.26.1 - 数据目录迁到 Hana 官方 plugin-data/<pluginId>：插件更新时 Hana 会整目录替换
// plugins/<id>（安装前自动备份到 plugin-backups/），数据放插件目录内会被删掉。
// plugin-data 与插件目录独立，更新插件不受影响；旧数据由 ensureDataDir() 自动迁移。
export const DATA_DIR = path.join(HANA_HOME, 'plugin-data', 'biaoqingbao');
...
// v0.30.1 - 读取 Hana 配置的用户名（展示「用户名话」用，ui.js 和 api.js 共用）
...
// 用户名话展示名：有用户名 → 「名字+话」；没有 → 兜底「我的风格」
export function userstyleDisplayName(userName) {
  return userName && String(userName).trim() ? String(userName).trim() + '话' : '我的风格';
}
