...
import { detectImageFormat } from './zip-images.js';
...
export async function prepareVisionImages(imageBase64, fileName) {
  ...
  const head = buffer.length >= 6 ? buffer.toString('ascii', 0, 6) : '';
  // v0.33.44 - 扩展名 .gif 但字节不是真 GIF（QQ 缓存文件名不可信）时：
  // 按真实签名探测格式降级静态图，不再抛 Invalid GIF header
  if (head !== 'GIF87a' && head !== 'GIF89a') {
    const real = detectImageFormat(buffer) || 'png';
    const mime = MIME_MAP[real] || 'image/png';
    return { images: [`data:${mime};base64,${rawBase64}`], animated: false, totalFrames: 1 };
  }

  const result = await extractGifFrames(buffer);
