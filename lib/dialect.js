...
export function buildDialectPersona(dialectId, _density) {
...
// 兼容别名（旧调用方）
export function buildDialectPrompt(dialectId, density) {
  return buildDialectPersona(dialectId, density);
}
...
function normalizeConfig(raw) {
  ...
  if (raw.agents && typeof raw.agents === 'object' && !Array.isArray(raw.agents)) {
    for (const [agentId, setting] of Object.entries(raw.agents)) {
      ...
      out.agents[agentId] = { dialect, enabled: true };
    }
  }
  return out;
}
...
export function applyDialectToIshiki(agentId, dialectId, density, agentsRoot = HANA_HOME) {
  const persona = buildDialectPersona(dialectId, density);
...
export function syncDialectToIshiki(config, agentsRoot = HANA_HOME, previousConfig = null) {
  ...
  for (const agentId of agentIds) {
    ...
    if (setting && getDialect(setting.dialect) && setting.enabled) {
      results[agentId] = applyDialectToIshiki(agentId, setting.dialect, 'on', agentsRoot);
    } else {
...
export function reconcileDialectToIshiki(config, agentsRoot = HANA_HOME) {
  ...
  for (const [agentId, setting] of Object.entries(norm.agents)) {
    ...
    try {
      ...
      const res = applyDialectToIshiki(agentId, setting.dialect, 'on', agentsRoot);
