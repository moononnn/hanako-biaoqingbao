...
// v0.30.0：学我说话需要读用户自定义模板（style-template.js 不依赖 dialect.js，单向依赖）
...
// v0.30.0 - 「学我说话」（userstyle）：第 10 种方言，但内容完全由用户自定义。
//   展示名固定「学我说话」，不随用户名动态拼。
...
export function buildDialectPersona(dialectId, _density, mode) {
  ...
  if (!d) return '';
  // v0.30.0：学我说话 = 用户自定义模板（动态），不走固定文案
...
export function buildDialectEcho(dialectId, randomValue = Math.random()) {
  const d = getDialect(dialectId);
  if (!d) return '';
  // v0.30.0：学我说话不额外回响——模板本身就是用户风格，不需要「拉回注意力」
...
// v0.30.11 - AGENTS.md 兼容（Hana agents 结构迁移）：
//   Hana 升级后（pre-agents-rename）人格文件从 ishiki.md 改为 AGENTS.md，
//   系统提示词改读 AGENTS.md。写入目标动态选择：AGENTS.md 存在（新结构）→ AGENTS.md；
//   不存在（旧结构 Hana）→ 退回 ishiki.md。所有读写/删除方言块的入口统一走这里，
//   保证「配置记忆」与「Hana 实际读的人格文件」一致（回归：保存四川话不生效，AGENTS.md 残留旧文案）。
//   路径安全复用 agentIshikiPath 的双保险校验（白名单 + 越界纵深），AGENTS.md 与 ishiki.md 同目录。
export function agentPersonaFilePath(agentId, agentsRoot = HANA_HOME) {
  const ishikiPath = agentIshikiPath(agentId, agentsRoot);
  const agentsFile = path.join(path.dirname(ishikiPath), 'AGENTS.md');
  if (fs.existsSync(agentsFile)) return agentsFile;
  return ishikiPath;
}
...
export function applyDialectToIshiki(agentId, dialectId, density, agentsRoot = HANA_HOME, mode = 'normal', opts = {}) {
  ...
  let filePath;
  try {
    filePath = agentPersonaFilePath(agentId, agentsRoot);
  } catch (e) {
...
export function readDialectFromIshiki(agentId, agentsRoot = HANA_HOME) {
  try {
    const existing = fs.readFileSync(agentPersonaFilePath(agentId, agentsRoot), 'utf-8');
...
//  已开启「学我说话」方言的助手 ishiki.md 里还是旧模板内容——重启后组装系统提示词
