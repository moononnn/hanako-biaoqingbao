...
### 7. 【重要！！！】方言口音会改动助手的意识栏
新版加了「方言口音」功能：给助手选一个方言后，插件会把对应的方言设定**写进该助手的意识栏（ishiki.md）**，让 ta 打字时自然带点家乡味。

- 写入的部分用标记块包起来，**只动插件自己的那一段，不会碰你写的内容**；
- 关闭方言后会自动移除，意识栏文件恢复原样；
- 改完需要**重启 Hana** 才生效。

这个机制本质上是在意识栏里注入一段「方言设定」提示词，不是给模型装了什么语音包，效果会因模型而异（新疆话就明确标注了「效果一般」）。
...
选方言其实就是给这位助手的意识栏注入一段对应的方言设定，让 ta 说话的习惯带上那味儿，聊天、接话、句尾语气都会自然一些。选择关闭方言后并重启hanako会自动清理注入的提示词，不留痕迹。

### 让助手模仿你说话（学我说话）

方言里还有一位特殊成员——「学我说话」。它不用挑方言，而是总结**你自己的说话习惯**：插件会分析你们的聊天记录（只取你说的话），提炼出一份「风格模板」，你确认后，助手就按这份模板模仿你说话。

- 默认分析全部助手那里的话，可以在「从哪些助手学」里取消勾选来排除；
- 模板可以编辑，也可以回退到上一版；
...
聊天里每张配图下方都有「喜欢 / 不喜欢」按钮，直接点就能反馈，插件会记录这些偏好，在以后选图时调整对应图片的权重。

不想要这排按钮的话，可以在「偏好设置 → 配图卡片」里关掉，卡片就只显示图片；想重新调教配图时随时打开。
...
插件主页右上角的「悬浮球」开关负责让桌面纸飞机出现或收起。纸飞机会轻轻漂浮，像在空中巡航；悬停时带着轻柔气体尾流并有流星划过背景，点击会蓄力冲刺，尾流和流星都会明显增强，再滑翔回航。右键悬浮球可以刷新表情包或关闭入口。想让哪张图出现在悬浮球里，去「表情包库」对应图片下点「加入悬浮球」；再次点击即可移出。悬浮球里的发送目标会跟随你最近正在聊天的窗口。
...
4. 配置识图模型，为图片生成描述和标签；
5. 去图库把想随手发的图加入悬浮球，回主页打开「悬浮球」开关；
6. 根据需要配置 `BAAI/bge-m3`，生成图库语义索引；
