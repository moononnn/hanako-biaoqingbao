...
在「伙伴配图设置」页，点开某位伙伴的「配置图库」，可以打开「全部图库」开关让 ta 用整库（以后新建的分组也自动算进来），也可以关掉开关按分组挑，并把挑中的分组标成「优先」。多个分组取并集；「优先」只给已勾选的分组加权，不会让没勾的图进来。
...
4. 配置识图模型，为图片生成描述和标签；
5. （可选）在图库新建分组，把图片放进一个或多个分组，再到「伙伴配图设置」里点「配置图库」，设置各伙伴的可用分组和优先使用；
