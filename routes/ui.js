...
function renderPage() {
  ...
  return '<!DOCTYPE html>'
    ...
    + '.group-weight-stars{display:inline-flex;align-items:center;gap:1px;margin-left:2px;flex:none}'
    + '.group-weight-star{padding:0 1px;border:0;background:transparent;color:var(--text-light);opacity:.4;cursor:pointer;font-size:11px;line-height:1.4;font-family:inherit;transition:all .15s}'
    + '.group-weight-star:hover:not(:disabled){opacity:1;color:#d08a9e}'
    + '.group-weight-star.is-lit{opacity:1;color:#c4708a}'
    + '.group-weight-star:disabled{opacity:.18;cursor:not-allowed}'
    ...
    + '<div class="form-hint" id="agent-library-hint">点星星给分组加权：1 星稍微偏一点，2 星偏爱，3 星主推（再点同一颗取消）。只在已经勾上的分组里生效，不会把没勾的图放进来；图同时属于多个加权分组时，按最重的那一档算。</div>'
    + '</div>'
