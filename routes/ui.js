...
function renderPage() {
  ...
  return '<!DOCTYPE html>'
    ...
    + '#view-agent-freq.global-off .agent-freq-item{opacity:.82}'
    + '#view-agent-freq.global-off .freq-options{pointer-events:none}'
    + '#view-agent-freq.global-off .scene-toggle-btn:disabled,#view-agent-freq.global-off .freq-enabled-btn:disabled,#view-agent-freq.global-off .freq-btn:disabled,#view-agent-freq.global-off .freq-del-btn:disabled,#view-agent-freq.global-off #save-agent-freq-btn:disabled{cursor:not-allowed}'
    ...
    + '<span class="entry-text"><b>伙伴配图频率</b><span>控制每个伙伴发图的频率高低</span></span>'
    + '<span class="entry-arrow">→</span>'
    + '</div>'
    ...
    + '</div>'
    + '<div class="pref-section">'
    + '<div class="section-desc">先决定这位伙伴是否可以配图，再分别设置日常聊天和处理正事时的频率。四档都是大致频率，插件会避免连续两轮提醒配图。</div>'
    + '<div class="global-freq-note" id="agent-freq-global-note" role="status">自动配图总闸已开启。关闭后会暂停自动情绪检测和伙伴自动发图，但不影响方言、识图和纸飞机。</div>'
    ...
    + '<button class="btn btn-ghost-freq" id="refresh-agents-btn" title="重新读取 Hana 当前配置的伙伴列表（新加的伙伴会出现在这里）">刷新列表</button>'
